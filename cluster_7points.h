...
//    }
    ...
    vector<int> min_cluster (vector<vector<int> > &graph, int given_id, const vector<Point>& cell_list)           //Given id and current cell list, compute the min weight edge
    {
        ...
        vector<int> a;
        int min_edge_id,min_edge_id2,min_edge_id3,min_edge_id4,min_edge_id5,min_edge_id6;
    //first round
    ...
    //fifth round
        ...
        for(int j=0; j<cell_list.size(); j++)
        {
            if( (graph[min_edge_id4][j] < min_edge) && (graph[min_edge_id4][j]!=0) && (graph[min_edge_id4][j]!=-1))
            {
                min_edge = graph[min_edge_id4][j];
                min_edge_id5 = j;
            }
        }
        for(int n=0; n<cell_list.size(); n++)
    ...
    //sixth round
        ...
        for(int j=0; j<cell_list.size(); j++)
        {
            if( (graph[min_edge_id5][j] < min_edge) && (graph[min_edge_id5][j]!=0) && (graph[min_edge_id5][j]!=-1))
            {
                min_edge = graph[min_edge_id5][j];
                min_edge_id6 = j;
            }
        }
        for(int n=0; n<cell_list.size(); n++)
        {
            graph[min_edge_id5][n] = -1;
            graph[n][min_edge_id5] = -1;
        }
    //seventh point invalid
        for(int n=0; n<cell_list.size(); n++)
        {
            graph[min_edge_id6][n] = -1;
            graph[n][min_edge_id6] = -1;
        }
        ...
        a.push_back(min_edge_id5);
        a.push_back(min_edge_id6);
        ...
        temp_cell[min_edge_id5].index = -1;
        temp_cell[min_edge_id6].index = -1;
    ...
    vector<vector<int>> strategy (vector<vector<int> > graph, const vector<Point>& whole_cell )  //greedy strategy
    {
        ...
        cout<<"N="<<whole_cell.size()/7<<endl;

        for(int i=0; i < whole_cell.size()/7; i++)     //based on cluster number
        {
            if (i%2 == 0)
            {
